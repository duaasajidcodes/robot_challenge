...
result1=$(echo -e "PLACE 0,0,NORTH\nMOVE\nREPORT" | docker run -i robot-challenge --cli | grep "0,1,NORTH" || echo "FAILED")
...
result2=$(echo -e "PLACE 0,0,NORTH\nLEFT\nREPORT" | docker run -i robot-challenge --cli | grep "0,0,WEST" || echo "FAILED")
...
result3=$(echo -e "PLACE 1,2,EAST\nMOVE\nMOVE\nLEFT\nMOVE\nREPORT" | docker run -i robot-challenge --cli | grep "3,3,NORTH" || echo "FAILED")
